        ...
        <div className="hidden lg:flex items-center gap-3 bg-[rgba(5,8,16,0.3)] border border-hp-border/50 px-3 py-1.5 rounded-sm font-mono">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-hp-text-secondary text-xs font-bold">
              {isLoading ? <span className="text-hp-text-muted">$...</span> : <>${avax.price.toFixed(2)}<span className={avax.change24h >= 0 ? "text-hp-accent-green text-[9px] ml-1" : "text-hp-accent-red text-[9px] ml-1"}>{avax.change24h >= 0 ? "+" : ""}{avax.change24h.toFixed(1)}%</span></>}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3 bg-[rgba(5,8,16,0.5)] border border-hp-border px-2 md:px-4 py-1.5 md:py-2 rounded-sm font-mono">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-hp-text-primary text-xs md:text-sm font-bold leading-tight">
              {isLoading ? <span className="text-hp-text-muted">$...</span> : <>${hcash.price < 1 ? hcash.price.toFixed(4) : hcash.price.toFixed(2)}<span className={hcash.change24h >= 0 ? "text-hp-accent-green text-[10px] md:text-xs ml-1" : "text-hp-accent-red text-[10px] md:text-xs ml-1"}>{hcash.change24h >= 0 ? "+" : ""}{hcash.change24h.toFixed(1)}%</span></>}
            </span>
          </div>
        </div>
      </div>
